...
| `BACKEND_ORIGIN` | For local dev | Backend origin used by the Vite dev proxy |
...
- `ai-interview-coach-web`: frontend web service that serves the Vite build and proxies `/api` to the backend
...
6. Set frontend `BACKEND_ORIGIN` to your backend Render URL, for example `https://ai-interview-coach-api.onrender.com`.
...
- Requests to `/api` are proxied from the frontend service to the backend public URL configured in `BACKEND_ORIGIN`.
...
- confirm frontend `BACKEND_ORIGIN` points to the backend Render URL
...
- frontend `BACKEND_ORIGIN` is correct for local dev or Render
