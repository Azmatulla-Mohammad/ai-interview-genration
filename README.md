...
- `ai-interview-coach-web`: frontend web service that serves the Vite build and proxies `/api` to the backend over Render's private network
- `ai-interview-coach-db`: Render Postgres database

1. Push this repo to GitHub.
2. In Render, choose **New +** -> **Blueprint**.
3. Connect this GitHub repository.
4. Render will detect `render.yaml` and propose the backend, frontend, and database services.
5. Provide `GEMINI_API_KEY` when prompted if you want Gemini enabled.
6. Create the Blueprint and wait for the initial deploy to finish.

### How This Render Setup Works

- The frontend service builds the Vite app and serves it from a small Node server.
- Browser requests stay same-origin on the frontend domain.
- Requests to `/api` are proxied from the frontend service to the backend using Render's private service hostname.
- The backend receives its `DATABASE_URL` from the Render Postgres instance.
- Render Postgres provides a standard `postgresql://...` connection string, and the backend normalizes it automatically for SQLAlchemy's `psycopg` driver.
